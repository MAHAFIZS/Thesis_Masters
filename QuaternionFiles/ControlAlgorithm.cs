    ...
    public class ControlAlgorithm : Block
    {
        ...
        override protected void OnNewInput(Block sender, object value)
        {
            ...
            if (sender == InputBlocks[0]) // Gesture input: expect int (or double cast to int)
            {
                ...
                switch (currentGesture)
                {
                    ...
                    default: Console.WriteLine($"Gesture {currentGesture} received, no axis lock change."); break;
                }
        ...
        private void ProcessControl(Vector Prediction, int gesture)
        {
            ...
            if (algorithm == Algorithms.Cartesian)
            {
                ...
                if (axisLocked != DOAs.None)
                {
                    // Increment or decrement Euler angles or position values
                    if (gesture == 4)
                    {
                        if (axisLocked == DOAs.phi) currentRoll += step;
                        else if (axisLocked == DOAs.theta) currentPitch += step;
                        else if (axisLocked == DOAs.psi) currentYaw += step;
                        else controlDict[axisLocked] = Math.Min(controlDict[axisLocked] + step, 1.0);
                    }
                    else if (gesture == 5)
                    {
                        if (axisLocked == DOAs.phi) currentRoll -= step;
                        ...
                        else controlDict[axisLocked] = Math.Max(controlDict[axisLocked] - step, -1.0);
                    }
                    else
                    {
                        if (axisLocked != DOAs.phi && axisLocked != DOAs.theta && axisLocked != DOAs.psi)
                            controlDict[axisLocked] = 0;
                    }

                    // Normalize angles to keep within [-π, π]
                    ...
                    double posX = controlDict.ContainsKey(DOAs.x) ? controlDict[DOAs.x] : 0.0;
                    double posY = controlDict.ContainsKey(DOAs.y) ? controlDict[DOAs.y] : 0.0;
                    double posZ = controlDict.ContainsKey(DOAs.z) ? controlDict[DOAs.z] : 0.0;

                    double[] outputPose = new double[] { posX, posY, posZ, q.qw, q.qx, q.qy, q.qz };

                    SendOutput(outputPose);

                    Console.WriteLine($"[ControlAlgorithm Output] Position: ({posX:F3}, {posY:F3}, {posZ:F3}), Quaternion: ({q.qw:F3}, {q.qx:F3}, {q.qy:F3}, {q.qz:F3})");
                }
                else
                {
                    // No axis locked: send predicted position and zero rotation (identity quaternion)
                    double[] outputPose = new double[7];
                    outputPose[0] = Prediction[0];
                    outputPose[1] = Prediction[1];
                    outputPose[2] = Prediction[2];
                    outputPose[3] = 1; // qw
                    outputPose[4] = 0; // qx
                    outputPose[5] = 0; // qy
                    outputPose[6] = 0; // qz
                    ...
                    Console.WriteLine($"[ControlAlgorithm Output - No Lock] Position: ({outputPose[0]:F3}, {outputPose[1]:F3}, {outputPose[2]:F3}), Quaternion: (1, 0, 0, 0)");
                }
            }
        }
